...
def info(bot, update):
    ...
    use them like so:
     ...
     WHERE:
    ` 
       [address] = withdraw #GOODS address
          [user] = telegram username 
        [amount] = amount of #GOODS to utilise 
    `

     *NOTE*:
      - don't deposit a significant amount of #GOODS through this #BOT
      - make sure that you enter a valid #GOODS address when you perform a withdraw
...
def tip(bot, update):
    ...
    if amount > 100000:
        msg = "Please send a lower amount (max: 100,000 GOODS)!"
        send_message(bot, chat_uuid, msg, False)
        return False
    if target == user_name:
    ...
    msg = '@{0} tipped @{1} of {2} GOODS'.format(user_name, target, amount)
    send_message(bot, chat_uuid, msg, False)
...
def balance(bot, update):
    ...
    msg = '@{0} your current balance is: {1} GOODS'.format(user_name, balance)
    send_message(bot, chat_uuid, msg, False)
...
def withdraw(bot, update):
    ...
    if not isValidAddress(message[1]):
        msg = 'Please input a valid GOODS address!'
        send_message(bot, chat_uuid, msg, False)
        return False
    if not isValidAmount(message[2]):
        msg = 'Please input a valid amount (ex: 1000)!'
    ...
    msg = '@{0} has successfully withdrew {2} GOODS to address: {1}'\
...
def price(bot, update):
    ...
    getmarket = 'https://chart.meanxtrade.com/info.php?market=GOODS_LTC'
